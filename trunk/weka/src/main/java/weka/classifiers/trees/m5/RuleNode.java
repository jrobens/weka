...
public class RuleNode extends AbstractClassifier {
  ...
  /**
   * Whether to use Kuenzel et al.'s split selection method.
   */
  ...
  public void split() throws Exception {
    ...
    if (!m_isLeaf) {
      ...
      if (!getKuenzelEtAlSplitSelection()) {
        bestSplit = new YongSplitInfo(0, m_numInstances - 1, -1);
        currentSplit = new YongSplitInfo(0, m_numInstances - 1, -1);
      } else {
        bestSplit = new RidgeRegressionSplitInfo(0, m_numInstances - 1, -1);
        currentSplit = new RidgeRegressionSplitInfo(0, m_numInstances - 1, -1);
      }
      ...
      if (bestSplit.splitAttr() < 0 || bestSplit.position() < 1
        ...
        m_left.setKuenzelEtAlSplitSelection(getKuenzelEtAlSplitSelection());
        ...
        m_right.setKuenzelEtAlSplitSelection(getKuenzelEtAlSplitSelection());
  ...
  /**
   * Use Kuenzel et al.'s split selection
   *
   * @param unpruned true if Kuenzel et al.'s split selection is used
   */
  public void setKuenzelEtAlSplitSelection(boolean unpruned) {
    m_useKuenzelEtAlSplitSelection = unpruned;
  }

  /**
   * Get whether Kuenzel et al.'s split selection is used
   *
   * @return true if Kuenzel et al.'s split selection is used
   */
  public boolean getKuenzelEtAlSplitSelection() {
    return m_useKuenzelEtAlSplitSelection;
  }
