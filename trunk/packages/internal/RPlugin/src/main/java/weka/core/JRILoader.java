...
public class JRILoader {
  ...
  /**
   *  Sets the environment variable in both the Java cache and the actual process environment
   */
  public static int setenv(String name, String value) {

    if (SetEnvironmentVariables.INSTANCE.setenv(name, value, 0) != 0) {
      System.err.println("Failed to set environment variable " + name + " in process environment");
      return 1;
    }
    return 0;
  }
  ...
  public static boolean checkRHome() {
    ...
    if (s_rHome == null) {
      ...
      if (javaHome == null) {
        ...
        if (setenv("JAVA_HOME", javaHome) != 0) {
          s_rHome = null;
          return false;
        }
      }
      ...
      if ((osType != null) && (osType.contains("Windows"))) {
        try {
          ...
          if (setenv("PATH", s_rHome + File.separator + "bin" + File.separator + subFolderName +
                  File.pathSeparator + PATH) != 0) {
            s_rHome = null;
            return false;
          }
          if (s_debug) {
      ...
      if (rLibsUser == null) { // Try to get library location if user has not set R_LIBS_USER
        ...
        if (rLibsUser == null) {
          try {
            ...
            if (!m.find()) {
              System.err.println("Failed to extract valid library folder from information returned by Sys.getenv(\"R_LIBS_USER\"): " +
                      firstLine);
              s_rHome = null;
              return false;
            }
        ...
        if (setenv("R_LIBS_USER", rLibsUser) != 0) {
          s_rHome = null;
          return false;
        }
      }
  ...
  private static Class<?> injectJRINativeLoader() throws Exception {
    ...
    byte[] byteCodeBasicRLogger = getByteCode("/weka/core/BasicRLogger.bytecode");
    ...
    try {
      ...
      defineClass.invoke(rootClassLoader, "weka.core.BasicRLogger",
              byteCodeBasicRLogger, 0, byteCodeBasicRLogger.length, pd);
