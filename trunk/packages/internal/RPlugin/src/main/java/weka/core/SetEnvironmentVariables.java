...
public class SetEnvironmentVariables {
    ...
    static {
        if (System.getProperty("os.name").contains("Windows")) {
            try {
                // Windows 10 or later should have this and R 4.2 or later for Windows uses UCRT!
                library = Native.loadLibrary("ucrtbase", Msvcrt.class);
            } catch (Throwable t) {
    ...
    interface Msvcrt extends Library { // These methods are the same in UCRT
