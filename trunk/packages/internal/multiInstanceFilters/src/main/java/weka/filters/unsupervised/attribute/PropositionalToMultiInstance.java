...
public class PropositionalToMultiInstance extends Filter implements
  ...
  /**
   * Checks whether the two class values are the same.
   *
   * @param class1	the first class value
   * @param class2	the second class value
   * @return		true if the same (or both missing)
   */
  protected boolean classValuesEqual(double class1, double class2) {
    if (Utils.isMissingValue(class1) && Utils.isMissingValue(class2))
      return true;
    else
  ...
  public boolean batchFinished() {
    ...
    for (int i = 0; i < input.numInstances(); i++) {
      ...
      if (currentBagIndex != bagIndex) {
        ...
        if ((input.classIndex() >= 0) && (!classValuesEqual(classValue, input.instance(i).classValue()))) {
